...
(let* ((script-directory
  ...
  (unless (= (length arguments) 1)
    (error "Expected one passive-locality request path, got ~S." arguments))
  (asdf:load-asd system-file)
  (asdf:load-system :cl-tpg)
  (handler-case
      (progn
        (funcall
         (intern "RUN-BEHAVIORAL-LOCALITY-SAMPLE-EVALUATION" :cl-tpg)
         (first arguments))
        (uiop:quit 0))
    (error (condition)
      (format *error-output*
              "~&Passive behavioral-locality evaluator failed: ~A~%"
              condition)
      (uiop:quit 1))))
